# Shooting hero and recording carousel

The hero now uses captured shooting frame 464 (mid-downswing), with 33 original
landmarks including MediaPipe's estimated depth. The face mesh and decorative
neck segment are removed. The ball uses its recorded screen position and radius;
its unmeasured depth is placed at the support foot. The source is regenerated by
`scripts/prepare-shooting-hero.mjs <ignored-source-folder>` with checksum validation.

Rotation runs automatically while visible, pauses during drag, and resumes 1.5 s
after release. Keyboard arrows remain available without on-screen buttons. System
reduced-motion settings suppress automatic rotation and initial playback.

The six recordings now advance in a carousel, wrapping after Shooting to Sprint.
Previous/next controls preserve a user's paused state; direct drill selection and
Watch rep retain their existing behavior. All playback controls use the same SVG
icon family as the cards, with separate play/pause shapes and accessible labels.

The introductory line is "For clubs, coaches, and player's" with the requested
apostrophe rendered typographically. The previous hero caption is removed.

Validation: 56 homepage/controller tests + 6 navigation tests passed; TypeScript
and scoped lint passed; Svelte reported no errors/warnings. Browser checks covered
WebGL rendering, absence of hero buttons, drag/pause/resume, carousel next/previous,
automatic Shooting-to-Sprint wrap, SVG state changes, 390px layout, text changes,
and no console errors. HTTP checks preserved all 157 application assets and 22
application routes. Other ongoing application changes were isolated from this release.

Preview: https://6aa709497b7306ba892ec85c--posetek.netlify.app

Earlier release notes below are historical where superseded.
