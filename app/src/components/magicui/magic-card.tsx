// Source: https://magicui.design/r/magic-card.json — MIT. Local changes: prefixed CSS, reduced motion.
"use client"
...
export function MagicCard(props: MagicCardProps) {
  ...
  return (
    <motion.div
      className={cn(
        "pt:group pt:relative pt:isolate pt:overflow-hidden pt:rounded-[inherit] pt:border pt:border-transparent",
        className
      )}
    ...
    >
      <div className="pt:bg-background pt:absolute pt:inset-px pt:z-20 pt:rounded-[inherit]" />

      {mode === "gradient" && (
        <motion.div
          suppressHydrationWarning
          className="pt:pointer-events-none pt:absolute pt:inset-px pt:z-30 pt:rounded-[inherit] pt:opacity-0 pt:transition-opacity pt:duration-300 pt:group-hover:opacity-100"
          style={{
            background: useMotionTemplate`
              radial-gradient(${gradientSize}px circle at ${mouseX}px ${mouseY}px,
                ${gradientColor},
      ...
      {mode === "orb" && (
        <motion.div
          suppressHydrationWarning
          aria-hidden="true"
          className="pt:pointer-events-none pt:absolute pt:z-30"
          style={{
            width: glowSize,
            height: glowSize,
            x: orbX,
      ...
      <div className="pt:relative pt:z-40">{children}</div>
    </motion.div>
  )
}
