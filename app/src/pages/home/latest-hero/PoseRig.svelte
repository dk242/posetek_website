<script lang="ts">
  ...
  let { points, opacity = 1 }: { points: readonly Point3[]; opacity?: number; layer?: HeroLayer } = $props();
  ...
  const pointMaterial = new MeshBasicMaterial({ color: '#ffffff', transparent: true, depthTest: true, depthWrite: false, toneMapped: false });
  function placePoints(mesh: InstancedMesh) {
    const matrix = new Matrix4();
    points.forEach(([x,y,z], index) => {
      const radius = index < 11 ? .004 : index > 16 && index < 23 ? .005 : .009;
...
<T.Mesh geometry={bones} renderOrder={3}>
  <T.MeshBasicMaterial vertexColors transparent {opacity} depthTest={true} depthWrite={false} toneMapped={false} />
</T.Mesh>
