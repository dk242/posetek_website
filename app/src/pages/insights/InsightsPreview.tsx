...
export default function InsightsPreview({ embedded = false }: { embedded?: boolean }) {
  ...
  const content = <>
    {!embedded && <div className="insights-preview-controls"><label>Preview role<select value={role} onChange={event => { setRole(event.target.value as InsightAccess); change({ orgId: event.target.value === "admin" ? undefined : "northfield", teamId: undefined }); }}><option value="admin">Admin</option><option value="manager">Manager</option><option value="coach">Assigned coach</option></select></label><label>Preview state<select value={state} onChange={event => { setState(event.target.value); setPage(0); }}><option value="normal">Recorded data</option><option value="empty">Empty roster</option><option value="uncollected">Usage not collected</option><option value="loading">Loading</option><option value="error">Load failure</option></select></label></div>}
    ...
    <InsightsControls choices={data.choices} request={request} scope={data.scope} loading={state === "loading"} hideScope={embedded} onChange={change} onRefresh={() => setState("normal")} />
    ...
    {state === "loading" ? <p role="status">Loading complete Insights…</p> : state === "error" ? <div role="alert"><p>Insights could not be loaded. Retry or choose another scope.</p><button className="quiet-button" onClick={() => setState("normal")}>Retry Insights</button></div> : <div id="insights-report" role="tabpanel" aria-labelledby={`insights-tab-${request.view}`}><ExpandedReport data={data} request={request} adminOverview={embedded && role === "admin"} onChange={change} page={page} onPrevious={() => setPage(value => value - 1)} onNext={() => setPage(value => value + 1)} playerLink={player => `#synthetic-${player.id}`} /></div>}
  </>;
