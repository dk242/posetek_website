...
import { SavedPlanDetails, recordedSessionLink } from './training-details';
...
export default function PlayerTraining({ ctx, statsProfile, request, onAcknowledge }: { ctx: PortalContext; statsProfile: Row; request: Row | null; onAcknowledge: () => void }) {
  ...
  if (review) {
    ...
    return <>{back}<p className="eyebrow">{log?.endedAt ? log.endReason === 'completed' ? 'Completed' : 'Ended early' : log ? 'In progress' : 'Your workout'}</p><h2>{snapshot.title || 'Workout'}</h2><p>{snapshot.intent}</p><p>{snapshot.estimatedMinutes} minutes · {snapshot.blocks?.length || 0} drills</p>{message}<WorkoutBlocks workout={snapshot} onDrill={setDrillId} />
      {log?.endedAt ? <><p>Saved {dateText(log.endedAt)}. Completed sets remain in your history.</p><Link to={preview ? '/feed?scope=mine&preview=1' : '/feed?scope=mine'}>Share your workout</Link></> : <button className="primary-cta" disabled={store.saving || !store.logsLoaded || (!log && plan?.status !== 'active')} onClick={() => void start(review)}>{store.saving ? 'Starting…' : log ? 'Resume workout' : 'Start workout'}</button>}
      {!log && plan?.status === 'active' && <button className="hub-secondary" disabled={!capabilityEnabled(config, 'workout_chat')} onClick={() => { setSeed(''); setChatTarget(review.source === 'plan' ? { kind: 'plan', planId: plan.id, workoutId: review.workoutId } : { kind: 'adhoc', plannedWorkoutId: review.workoutId }); setReview(null); }}>Adjust with your coach</button>}
    </>;
  }
  ...
  if (panel === 'details') return <>{back}<section className="portal-card"><SavedPlanDetails plan={plan} /><button className="hub-secondary" onClick={openPlan}>Start a new plan</button></section></>;
  return <section className="player-training"><p className="eyebrow">Your training</p><h1>Make every session count.</h1>{message}
    ...
    {(unfinished || next) && <button className="primary-cta player-next" disabled={!store.logsLoaded} onClick={() => setReview(unfinished || executable(plan, next!.workout, next!.weekNumber))}><span><small>{unfinished ? unfinished.planId === plan.id ? 'Resume saved workout' : 'Resume saved workout · earlier plan' : next!.reason === 'resume' ? 'Resume' : `Next · Week ${next!.weekNumber}`}</small><strong>{unfinished?.title || next?.workout.title}</strong></span><span>→</span></button>}
...
function WeekProgress({ plan, week, logs, reps, sessions }: { plan: Row; week: any; logs: Row[]; reps: Row[]; sessions: Row[] }) {
  ...
  return <section className="portal-card"><h2>This week’s progress</h2>{progress.domains.map(d => <div className="player-progress-row" key={d.domain}><span>{domainLabel(d.domain)}{d.targetMinutes > 0 && <small> · {d.minutes}/{d.targetMinutes} min · {d.minutesRemaining} min remaining</small>}</span><strong>{d.done}/{d.target}</strong><progress aria-label={`${domainLabel(d.domain)} session progress`} max={Math.max(1, d.target)} value={d.done} /></div>)}<p><strong>{progress.minutes} minutes</strong> credited from completed and partial drills.</p></section>;
}
