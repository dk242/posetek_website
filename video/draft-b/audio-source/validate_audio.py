...
for name,channels in [('narration.wav',1),('bed.wav',2),('master.wav',2)]:
    ...
    for interval in spec.get('silent_intervals', []):
        reserved = data[round(interval['start'] * sr):round(interval['end'] * sr)]
        assert np.count_nonzero(reserved) == 0, f'{name}: sound in reserved interval'
...
for interval in spec.get('silent_intervals', []):
    assert all(c['end'] <= interval['start'] or c['start'] >= interval['end'] for c in captions), 'Narration caption overlaps original-audio interval'
...
if spec.get('silent_intervals'):
    summary['silent_intervals_verified'] = spec['silent_intervals']
