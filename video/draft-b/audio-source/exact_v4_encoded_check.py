"""Validate the once-encoded V4 audio against its fully mixed PCM reference.

Usage: python exact_v4_encoded_check.py path/to/final.mp4 [--output receipt.json]
...
from exact_v4_mix import ROOT, OUT, SR, digest, dump, meter
...
def main():
    ...
    assert len(data) >= len(source_data), 'AAC is shorter than the 113-second program'
    ...
    windows = {
        ...
        'closing_narration': (100, 104),
        'music_only_coda': (106, 111),
    }
    ...
    report = {
        'passed': True, 'version': 'investor-exact-v4', 'movie': str(movie),
        ...
        'caption_source_unchanged': manifest['captions']['exact_v3_wording'],
    }
    dump(receipt, report)
