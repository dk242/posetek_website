...
const directory=resolve('output/investor-exact-v4');
const source=resolve(directory,'PoseTek-Investor-Exact-V4-1080p.mp4');
const target=resolve(directory,'PoseTek-Investor-Exact-V4-Email-Review.mp4');
...
if(Math.abs(Number(probe.format.duration)-113)>.05)throw new Error('Expected complete 113-second master');
const common=['-hide_banner','-loglevel','error','-y','-i',source,'-map','0:v:0','-vf','scale=640:360:flags=lanczos,fps=15','-c:v','libx264','-preset','slow','-b:v','140k','-pix_fmt','yuv420p','-passlogfile',resolve(directory,'review-pass')];
