...
export const EXACT_TECHNIQUE_FRAMES=840;
export const EXACT_KICK_REVIEW_FRAMES=240;
export function exactTechniqueState(frame:number){
  if(frame<48)return {index:0,step:0,mode:'ready' as const};
  if(frame<67)return {index:frame-48,step:0,mode:'approaching' as const};
  if(frame<220)return {index:19,step:0,mode:'paused' as const};
  if(frame<227)return {index:19+frame-220,step:1,mode:'approaching' as const};
  if(frame<415)return {index:26,step:1,mode:'paused' as const};
  // There is deliberately no transition, playback or pose interpolation here.
  if(frame<601)return {index:26,step:2,mode:'paused' as const};
  if(frame<626)return {index:26+frame-601,step:3,mode:'approaching' as const};
  if(frame<786)return {index:51,step:3,mode:'paused' as const};
...
export const ExactTechnique:React.FC<Props>=({frame})=>{
  ...
  const data=source.technique,state=exactTechniqueState(frame),step=prepared.steps[state.step];
  ...
  const speedMenu=frame>=12&&frame<37,quarter=frame>=36;
  ...
  return <div style={outer}>
    ...
    <div style={{position:'absolute',left:30,top:627,width:PLOT-60,height:43,display:'flex',alignItems:'center',gap:25}}>
      ...
      <div style={{position:'relative',width:79,padding:'8px 0',background:'#153128',borderRadius:7,textAlign:'center',fontFamily:mono,fontSize:18}}>{quarter?'¼×':'1×'} ▾
        {speedMenu&&<div style={{position:'absolute',bottom:51,left:-6,width:126,padding:6,background:'#173a2b',border:`1px solid #42614e`,borderRadius:10,boxShadow:'0 10px 35px #0008'}}>{['¼×','½×','1×'].map((rate,i)=><div key={rate} style={{padding:'9px 13px',textAlign:'left',background:frame>=30&&i===0?C.lime:'transparent',color:frame>=30&&i===0?C.bg:C.ink,borderRadius:6}}>{rate}{i===2&&frame<36?' ✓':''}</div>)}</div>}
      </div>
    ...
    <Pointer frame={frame} at={35} x={109} y={507} tail={3}/><Pointer frame={frame} at={47} x={40} y={648} lead={7} tail={8}/>
    {[219,414,600,785].map(at=><Pointer key={at} frame={frame} at={at} x={1397} y={638}/>)}
  </div>;
};
 ...
 */
export const ExactKickReview:React.FC<Props>=({frame})=>{
  const source=useSource();
  ...
  if(!source||!prepared)return null;
  const paused=frame>=50,observation=source.comparison.observation;
  return <div style={outer}>
    ...
    {prepared.map(({record,project},i)=>{
      const local=Math.min(record.keyFrames.backswing-record.startFrame,Math.floor(Math.max(0,frame)*record.fps*.25/30));
