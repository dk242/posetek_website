...
/** V4: 444-frame walkthrough. Recorded motion is unchanged; only cue holds are shorter.
...
export const EXACT_TECHNIQUE_V4_FRAMES=timing.technique.duration_frames;

export function exactTechniqueStateV4(frame:number){
  const t=timing.technique;
  if(frame<t.ready[1])return {index:0,step:0,mode:'ready' as const};
  if(frame<t.approach_backswing[1])return {index:frame-t.approach_backswing[0],step:0,mode:'approaching' as const};
  if(frame<t.backswing[1])return {index:19,step:0,mode:'paused' as const};
  if(frame<t.approach_contact[1])return {index:19+frame-t.approach_contact[0],step:1,mode:'approaching' as const};
  if(frame<t.support_foot[1])return {index:26,step:1,mode:'paused' as const};
  if(frame<t.arm_balance[1])return {index:26,step:2,mode:'paused' as const};
  if(frame<t.approach_followthrough[1])return {index:26+frame-t.approach_followthrough[0],step:3,mode:'approaching' as const};
  if(frame<t.followthrough[1])return {index:51,step:3,mode:'paused' as const};
...
export const ExactTechniqueV4:React.FC<Props>=({frame})=>{
  ...
  const data=source.technique,state=exactTechniqueStateV4(frame),step=prepared.steps[state.step];
  ...
  return <div style={outer}>
    ...
    {timing.technique.continue_frames.map(at=><Pointer key={at} frame={frame} at={at} x={1397} y={638}/>)}
  </div>;
};
