...
export function EvidenceV3({f}:{f:number}){
 // Window starts at 3.8s: all six recorded motions enter together, and the
 // containers complete their formation from 6.7s to 7.2s. Playback stays 1x.
 const boxes=p(f,87,102),arrive=p(f,0,12);
 return <>
  ...
  <div style={{position:'absolute',left:76,right:76,top:535,display:'grid',gridTemplateColumns:'1fr 1fr',gap:24,opacity:arrive}}>
   {names.map((name,i)=><div key={name} style={{height:330,position:'relative',overflow:'hidden',borderRadius:16,background:`rgba(10,33,26,${boxes})`,transform:`translateY(${(1-arrive)*(i%2?32:50)}px)`}}>
    <div style={{position:'absolute',inset:0}}><MovementPreview index={i} f={Math.max(0,f)}/></div>
...
export function ClubV3({f}:{f:number}){
...
export function TrainingV3({f}:{f:number}){
...
export function ConnectedV3(){return <>
