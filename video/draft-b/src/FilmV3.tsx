...
import {EvidenceV3,ClubV3,TrainingV3,ConnectedV3} from './RevisionScenesV3';
...
function Window({start,end,children}:{start:number;end:number;children:(f:number)=>React.ReactNode}){
 ...
 const enter=p(f,0,14),exit=end===1560?1:1-p(frame,end-14,end);
...
function Closing({f}:{f:number}){
 return <>
  ...
  <Reveal frame={f} at={4} style={{position:'absolute',left:76,right:76,top:635}}><h2 style={{...heading,fontSize:130}}>Start with<br/>evidence.<br/><span style={{color:C.lime}}>Train what’s next.</span></h2></Reveal>
  <Reveal frame={f} at={10} style={{position:'absolute',top:1090,left:76,right:76}}><Rule/><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',paddingTop:36}}><span style={{fontSize:43,fontWeight:600}}>posetek.net/coaches</span><div style={{background:C.lime,width:76,height:76,borderRadius:40,display:'grid',placeItems:'center'}}><Arrow color={C.bg}/></div></div></Reveal>
 </>;
}

export const FilmV3:React.FC<Props>=({audioEnabled=false,captions=[]})=>{
 const frame=useCurrentFrame();
 const cap=captions.find(c=>frame/30>=c.start&&frame/30<c.end);
 const chapter=frame<120?'THE QUESTION':frame<270?'ASSESS':frame<630?'UNDERSTAND':frame<1050?'TRAIN':frame<1290?'RETEST':frame<1410?'DEVELOP':'POSETEK';
 return <AbsoluteFill style={{background:C.bg,color:C.ink,fontFamily:'Inter, sans-serif',overflow:'hidden'}}>
  ...
  <div style={{position:'absolute',zIndex:40,top:71,left:76,right:76,display:'flex',justifyContent:'space-between',alignItems:'center',opacity:1-p(frame,1402,1420)}}><Brand/><Eyebrow color={C.muted} style={{fontSize:19,letterSpacing:2}}>FOR CLUBS & COACHES</Eyebrow></div>
  <Window start={0} end={126}>{f=><Hero f={f}/>}</Window>
  <Window start={114} end={276}>{f=><EvidenceV3 f={f}/>}</Window>
  <Window start={264} end={636}>{f=><ClubV3 f={f}/>}</Window>
  <Sequence from={624} durationInFrames={432} layout="none"><TrainingLocal/></Sequence>
  <Window start={1044} end={1296}>{f=><Retest f={f}/>}</Window>
  <Window start={1284} end={1416}>{()=> <ConnectedV3/>}</Window>
  <Window start={1404} end={1560}>{f=><Closing f={f}/>}</Window>
  ...
  <div style={{position:'absolute',left:76,right:76,bottom:70,height:2,background:C.line}}><div style={{width:`${frame/1559*100}%`,height:2,background:C.lime}}/></div>
  ...
  {audioEnabled&&<Audio src={staticFile('audio-v3/master.wav')}/>}
 </AbsoluteFill>;
};
function TrainingLocal(){const f=useCurrentFrame();const alpha=p(f,0,14)*(1-p(f,418,432));return <AbsoluteFill style={{opacity:alpha,transform:`translateY(${(1-p(f,0,14))*30}px)`}}><TrainingV3 f={f}/></AbsoluteFill>;}
