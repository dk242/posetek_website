...
export function useProfileV2():Profile {
...
export function ExactPlayerProfileV2({frame}:ProductScreenProps) {
  ...
  return <div style={box}>
    ...
    <div style={{position:'absolute',left:772,top:137,right:48}}>
      ...
      {axes.map((a,i)=><div key={a.id} style={{padding:'13px 16px',marginBottom:6,background:chosen===i?C.raised:'transparent',border:`1px solid ${chosen===i?C.lime:C.line}`,borderRadius:9,display:'flex',alignItems:'center',gap:20,minHeight:66,boxSizing:'border-box'}}><div style={{width:148,fontSize:24,fontWeight:600,color:chosen===i?C.lime:C.ink}}>{a.label}</div><div style={{flex:1}}>{(a.metrics??[]).map((m,j)=><div key={j} style={{display:'flex',justifyContent:'space-between',gap:15,fontSize:21,lineHeight:1.5}}><span style={{color:C.muted}}>{m.label}</span><span style={{whiteSpace:'nowrap',color:C.lime}}>{m.value} <span style={{fontSize:16,color:C.muted}}>{m.unit}</span></span></div>)}</div></div>)}
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:18,fontSize:24,color:chosen===3?C.lime:C.muted}}><span>{chosen===3?'Selected focus · Close control & passing':'Measured results support the skill map'}</span><Arrow size={28}/></div>
    </div>
...
export function ExactPlanBuilderV2({frame}:ProductScreenProps) {
...
export function ExactGuidedWorkoutV2({frame}:ProductScreenProps) {
  const complete=frame>=88;const next=frame>=123;const videoFrame=next?frame-123:frame;
  ...
  return <div style={box}>
    ...
    <Tap frame={frame} at={87} x={873} y={568}/><Tap frame={frame} at={122} x={873} y={568}/>
...
export function ExactProgressReviewV2({frame}:ProductScreenProps) {
  const profile=useProfileV2();const history=(profile.history??[]).slice(0,3);const phase=Math.min(2,Math.floor(frame/95));
  return <div style={box}>
