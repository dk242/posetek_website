...
function useProfile():Profile {
  ...
  useEffect(()=>{
    let active=true;
    fetch(staticFile('product/profile.json'))
...
export function PlayerProfile({frame}:ProductScreenProps) {
  ...
  const selected=available.length?available[Math.min(available.length-1,Math.floor(frame/72))].i:Math.min(4,Math.floor(frame/72));
  const axis=axes[selected];const metrics=axis.metrics??profile.metrics??[];
  return <div style={box}>
    ...
    <div style={{position:'absolute',left:28,top:117,width:694,height:523,borderRight:`1px solid ${C.line}`}}><Micro style={{position:'absolute',left:26,top:3,color:C.lime}}>SKILL MAP</Micro><Radar axes={axes} selected={selected}/><Micro style={{position:'absolute',left:27,bottom:0,fontSize:17}}>Default projected reference · 100 = reference</Micro></div>
    <div style={{position:'absolute',left:772,top:137,right:48}}>
      <div style={{display:'flex',gap:9,marginBottom:24}}>{axes.map((a,i)=><Tag key={a.id} active={i===selected}>{a.label}</Tag>)}</div>
    ...
    <Footer>Reconstructed native profile · Default projected reference · 100 = reference, not a percentile · Recorded results remain separate from scores.</Footer>
  </div>;
}

const workoutFocus='I have 15 minutes. Help me work on close control and passing.';
export function PlanBuilder({frame}:ProductScreenProps) {
  ...
  return <div style={box}>
    ...
    <div style={{position:'absolute',left:34,top:133,width:358,bottom:65,borderRight:`1px solid ${C.line}`,paddingRight:30,boxSizing:'border-box'}}>
      <Micro style={{color:C.lime}}>TRAINING</Micro><h3 style={{...display,fontSize:52,marginTop:15}}>{plan?<>A plan with<br/>a clear focus.</>:<>Shape today’s<br/>session.</>}</h3>
...
export function GuidedProductWorkout({frame}:ProductScreenProps) {
...
export function CoachingConversation({frame}:ProductScreenProps) {
  const profile=useProfile();const saved=profile.coaching?.cue;
  const question=profile.coaching?.question??'How should I use this comparison in my next session?';
  const answer=profile.coaching?.answer??'Start with one observation from the saved review. Practise it at a comfortable pace, then record another kick from the same camera setup to compare. Ask your coach which cue to prioritise.';
  const typedQuestion=question.slice(0,Math.max(0,Math.floor(frame*3)));
  const sent=frame>=28;const answering=frame>=40;const shownAnswer=answer.slice(0,Math.max(0,Math.floor((frame-40)*16)));
  return <div style={box}>
    <Header section="AI Coach" title="One useful step at a time" right={<Tag>{profile.displayName?'Training questions for '+profile.displayName:'Training questions'}</Tag>}/>
    <div style={{position:'absolute',left:34,top:140,width:545,bottom:70,paddingRight:33,borderRight:`1px solid ${C.line}`,boxSizing:'border-box'}}>
      <Micro style={{color:C.lime}}>FROM THE TECHNIQUE REVIEW</Micro><h3 style={{...display,fontSize:57,marginTop:17}}>Understand<br/>the next step.</h3>
      <div style={{marginTop:26,borderLeft:`3px solid ${C.lime}`,paddingLeft:23,fontSize:25,lineHeight:1.48}}>{saved??'Compare the recorded kicks at the same movement phase.'}</div>
      <Micro style={{marginTop:24,fontSize:17}}>{profile.coaching?.sourceLabel??'Use the saved comparison as your starting point.'}</Micro>
    </div>
    <div style={{position:'absolute',left:629,top:136,right:40,bottom:64}}>
      <div style={{marginLeft:155,background:C.raised,border:`1px solid ${C.line}`,borderRadius:'16px 16px 3px 16px',padding:'19px 24px',fontSize:27,lineHeight:1.42,minHeight:102,boxSizing:'border-box'}}>{sent?question:typedQuestion}{!sent&&<span style={{color:C.lime}}>│</span>}</div>
      {sent&&<div style={{marginTop:25,display:'flex',gap:19}}><div style={{width:40,height:40,flexShrink:0,background:C.lime,color:C.bg,borderRadius:11,display:'grid',placeItems:'center',fontSize:25,fontWeight:600}}>P</div><div style={{flex:1}}><Micro style={{color:C.lime,fontSize:17}}>POSETEK COACH</Micro><div style={{fontSize:27,lineHeight:1.5,marginTop:11,minHeight:182}}>{answering?shownAnswer:<span style={{color:C.muted}}>Thinking through your question…</span>}</div></div></div>}
      <div style={{position:'absolute',left:0,right:0,bottom:4,height:62,border:`1px solid ${C.line}`,borderRadius:12,display:'flex',justifyContent:'space-between',alignItems:'center',padding:'0 20px',fontSize:23,color:C.muted}}><span>Ask your coach…</span><div style={{height:37,width:37,borderRadius:19,background:C.lime,color:C.bg,display:'grid',placeItems:'center'}}><Arrow size={22}/></div></div>
    </div>
    <Footer>{saved?'Saved review cue':'Technique-review context'} · Reconstructed native chat · Illustrative coaching conversation; no live AI request or new analysis is shown.</Footer>
  </div>;
}

export function ProgressReview({frame}:ProductScreenProps) {
  ...
  return <div style={box}>
    ...
    <div style={{position:'absolute',left:35,top:139,width:695,bottom:65,paddingRight:40,borderRight:`1px solid ${C.line}`,boxSizing:'border-box'}}>
      <Micro style={{color:C.lime}}>RECORDED BASELINE</Micro><h3 style={{...display,fontSize:54,marginTop:13}}>Keep the evidence.<br/>Return to the test.</h3>
