...
The merged source contains both the personalized planner and newer kick-comparison tools. Run `npm --prefix app run build` (or the compatible `node scripts/build-production.mjs`) and publish `dist`, as configured in `netlify.toml`. See `PRODUCTION_INTEGRATION.md` for the unified routing and validation workflow.
