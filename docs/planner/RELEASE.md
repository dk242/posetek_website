...
**Source integration, 2026-09-09:** the planner and kick-review histories are reconciled. The current build publishes one application from `dist`; the split-entry deployment descriptions below are historical. See [PRODUCTION_INTEGRATION.md](PRODUCTION_INTEGRATION.md) for current commands and [integration verification](../../deployments/source-integration-2026-09-09.md) for acceptance results.
...
The follow-up admin refresh was published September 8, 2026 as Netlify deployment `6aa08cd30ebacfbe76494bb9`, from source commit `2fcf6c5`. All main admin routes now serve the approved refreshed components, including roster signup codes, initials, mobile bottom tabs and both planners. The newer technique-review and results/rep workspaces retain their serving application and receive the shared admin theme. The current planner remains the default and fallback. Technique review is available from admin home and the header; personalized planning is available from home, Programs and player details.

The mobile reference was checked against `posetek-mobile-app` origin/main `1841566`, specifically `CoachSurface`, `PoseTekProfileBackground` and `AdminShellView`. TypeScript and all 558 tests passed; 36 route/viewport fixtures and planner review/fallback fixtures passed at 1440, 820 and 390px. The final deployed candidate passed 19 route rewrite checks, both real application gates, document/history navigation, four mobile tabs and non-admin style isolation. The existing Vacaville logo returned 200 and decoded in the browser. Published-file verification confirmed all 152 original non-index served files remain checksum-identical and the 65 added admin files match the tested build. This rollout changes no player plan or completion record.

The initial planner-only integration was published at 22:08 UTC as deployment `6aa087784119565fc043dabf`, from `6568246`, followed by the main-branch automatic build `6aa087f5f3869f0009cf6ac9`. That first integration retained older main-admin UI; the follow-up above corrects it. Fixed preview URLs remain snapshots and do not automatically show current production changes.
